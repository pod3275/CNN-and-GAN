...
Discriminator_result_real_image, Discriminator_l2_real_image = Discriminator(IMAGE_2D)
Discriminator_result_generated_image, Discriminator_l2_generated_image = Discriminator(Generated_image, True)

####### Losses
D_loss = -tf.reduce_mean(tf.log(Discriminator_result_real_image) + tf.log(1 - Discriminator_result_generated_image))
G_loss = -tf.reduce_mean(tf.log(Discriminator_result_generated_image)) + tf.reduce_mean(tf.sqrt(tf.square(Discriminator_l2_generated_image)))
...
for epoch in range(201):
    ...
    for i in range(total_batch):
        ...
        _, g_loss_val = sess.run([G_optimizer, G_loss], feed_dict={IMAGE: batch_xs, NOISE: batch_noises, IS_TRAINING: True})
